...
> 1. **pm** (main session, opus) — gathers requirements, owns the PRD + business outcomes + token build-budget decision + conflict resolution, enforces all gates, dispatches everyone. Connectors: slack, jira, notion.
> 2. **product-designer** (sonnet) — UX spec + UI mocks to Notion from the approved PRD; owns UX. Connectors: notion, slack.
> 3. **software-engineer** (opus) — implements app logic, APIs, data access via TDD; **owns maintainability + app architecture**; receives all defect/review/app-security/infra-cost feedback. Connectors: github, jira, notion, slack.
> 4. **frontend-engineer** (sonnet) — builds the ReactJS UI pixel-perfect to mocks, responsive/cross-browser, OAuth2 client (PKCE, no secrets in bundle). Connectors: github, jira, notion, slack.
...
> 6. **devops-engineer** (opus) — CloudFormation infra, staging+prod deploys, monitoring/alerting to Slack; **owns infra run cost and rollbacks + a runbook in Notion**; estimates monthly AWS cost (staging + production) for the PRD. Connectors: aws, github, slack, jira.
> 7. **data-engineer** (sonnet) — instruments business/usage/ROI metrics, fabricates safe test data, and **owns DB migrations** (checked into a `migrations` location, reviewed in the same PR as the code). Connectors: aws, github, notion, slack.
> 8. **support-writer** (sonnet) — triages user feedback into Jira (bug vs enhancement) and writes docs/training to Notion. Connectors: jira, notion, slack.
...
>
> **6. Systems of record** (MCP connectors, referenced by name): Slack (human comms + observability), GitHub (code in locations `application`, `api`, `ux`, `tests`, `infrastructure`, `migrations`), Notion (PRD, plans, mocks, runbook, docs), Jira (feature + bug tickets), AWS (infra, deploys, monitoring, secrets).
>
